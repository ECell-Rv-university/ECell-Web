...
function formatCardDate(event: EventItem): {
  day: string;
  month: string;
} {
  if (event.date === "TBA") {
...
export default function EventsArchive(): React.ReactElement {
  ...
  const archivedEvents = useMemo(
    () => EVENTS.filter((event) => event.status === "ARCHIVE"),
    [],
  );
  ...
  useEffect(() => {
    ...
    /*
     * Wait for the browser to complete one layout pass
     * before GSAP calculates animation positions.
     */
